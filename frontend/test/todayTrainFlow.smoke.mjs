...
assert(!dashboard.includes("openRoutePlanner: true"), 'Dashboard no longer owns Today route planning')
assert(!dashboard.includes('todayWatchWorkout'), 'Dashboard no longer constructs duplicate watch content for Today')
assert(!dashboard.includes('hasRunRecordedToday={hasRunRecordedToday}'), 'Dashboard passes no recorded-run state into removed Today surfaces')
assert(dashboard.includes('isRunningActivity(run) && runOccurredOnDate(run, todayISO)'), 'travel safeguards continue to exclude walks and other non-running activities')
assert.equal((dashboard.match(/hasRunRecordedToday=\{hasRunRecordedToday\}/g) || []).length, 0, 'removed Today card and detail sheet receive no Dashboard state')
...
assert(!dashboard.includes('authorizeWorkoutStart'), 'Dashboard owns no workout start path after Today removal')
