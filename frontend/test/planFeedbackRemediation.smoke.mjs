...
assert(!dashboard.includes('onStartUnplannedRun={handleStartUnplannedRun}'), 'Dashboard leaves the Today rest action in Train')
assert(!dashboard.includes("navigate('/log-run?tab=manual&intent=rest-day')"), 'Dashboard does not duplicate the Train rest-day intent chooser')
...
assert(!dashboard.includes('<TodayDetailSheet') && !dashboard.includes('onStartRun={handleStartTodayRun}') && !dashboard.includes('onStartLift={handleStartTodayLift}'), 'Dashboard removes its duplicate Today details and start handlers')
