...
check(community.includes('setInviteError(t(\'community.inviteHandleRequired\'))'), 'missing invite prerequisites render feedback beside the invite control')
check(community.includes("t('community.setupInviteHandle')") && community.includes("setFriendDiscoveryMode('handle')"), 'inline invite feedback sends the athlete directly to handle setup')
check(community.includes('inviteHandleInputRef.current?.scrollIntoView') && community.includes("[friendDiscoveryMode]"), 'handle setup scroll waits for the Handle tab to render')
