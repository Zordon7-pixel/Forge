...
const directDelete = races.indexOf('api.delete(`/races/${encodeURIComponent(race.id)}`')
...
assert.match(races, /if \(data\.requires_apply\)[\s\S]*candidate_hash:[\s\S]*choice:\s*'train_for_target'/, 'linked removal applies the exact candidate token returned behind the action')
