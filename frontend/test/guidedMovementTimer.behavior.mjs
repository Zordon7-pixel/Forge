...
assert.deepEqual(reduce(state, TIMER_ACTION.PAUSE), state, 'duplicate native and web pause signals are idempotent')
state = reduce(state, TIMER_ACTION.START)
...
assert.match(warmupSource, /const pauseCountdown[\s\S]{0,120}TIMER_ACTION\.PAUSE/, 'lifecycle pause signals dispatch the timer pause action')
assert.match(warmupSource, /await CapacitorApp\.addListener\(\s*'appStateChange',[\s\S]{0,200}!isActive\) pauseCountdown\(\)/, 'native app backgrounding pauses the warm-up countdown')
assert.match(warmupSource, /if \(!active\)\s*\{\s*await removeAppStateListener\(handle\)/, 'late native listener registration is cleaned up after unmount')
