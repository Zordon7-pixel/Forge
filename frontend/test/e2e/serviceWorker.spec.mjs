...
test('service worker purges old caches and never stores or serves HTML as JavaScript', async ({ page, context }) => {
  ...
  await expect.poll(() => page.evaluate(async () => {
    ...
    return names.includes('forge-v9') && !names.includes('forge-v8') && !names.includes('forge-v5') && !names.includes('forge-v6') && !names.includes('forge-v7') && !names.includes('forge-api-v1')
  })).toBe(true)
  const cacheNames = await page.evaluate(() => caches.keys())
  expect(cacheNames).toContain('forge-v9')
...
test('a rendered login page survives an immediate offline reload', async ({ page, context }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Log In', exact: true })).toBeVisible()
  await expect.poll(() => page.evaluate(async () => {
    const cache = await caches.open('forge-v9')
    const codeUrls = [
