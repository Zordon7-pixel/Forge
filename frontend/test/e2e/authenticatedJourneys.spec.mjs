...
test('Signature UI opens canonical readiness on demand and keeps Coach\'s Daily Brief first on both mobile projects', async ({ page }, testInfo) => {
  ...
  const briefToggle = coachsLog.locator('.signature-log-toggle')
  await expect(briefToggle).toHaveAccessibleName("Open Coach's daily brief for Controlled progression run")
  await expect(briefToggle).toHaveAttribute('aria-expanded', 'false')
  await expect(coachsLog.locator('[data-mission-fact="duration"]')).toBeHidden()
  ...
  await expect(briefToggle).toHaveAccessibleName("Close Coach's daily brief for Controlled progression run")
  ...
  await expect(coachsLog.locator('[data-brief-context="readiness"] dd').first()).toHaveText('83 · Amber')
  await expect(coachsLog.getByText('Sleep duration supports today.', { exact: true })).toBeVisible()
