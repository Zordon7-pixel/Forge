...
async function expectNoRoutineCheckInControls(page) {
  await expect(page.getByRole('button', { name: /^(?:Check in|Edit check-in|Go to Check-In)$/i })).toHaveCount(0)
  await expect(page.getByRole('heading', { name: 'Morning Check-In Required' })).toHaveCount(0)
}
...
test('Signature UI opens canonical readiness on demand and keeps Coach\'s Daily Brief first on both mobile projects', async ({ page }, testInfo) => {
  ...
  const apiState = await installAuthenticatedApi(page, {
    token: createQaToken({ exp: Math.floor(fixedNow.getTime() / 1000) + 3600 }),
    responses: new Map([
  ...
  const dailyPlanRequests = requestsFor(apiState, 'GET', '/api/plans/today')
  expect(dailyPlanRequests, 'Today intentionally reads current and next local dates for first-class Tomorrow planning').toHaveLength(2)
  expect(dailyPlanRequests.map((request) => request.search.date).sort()).toEqual([today, tomorrow].sort())
...
test('planned rest day stays visible and opens an optional extra run without a questionnaire', async ({ page }) => {
  const runtimeErrors = collectRuntimeErrors(page)
  const apiState = await installAuthenticatedApi(page, {
    responses: new Map([
  ...
  await expectNoRoutineCheckInControls(page)
  ...
  await expect(page.getByRole('button', { name: 'Choose Run', exact: true })).toBeVisible()
  await expectNoRoutineCheckInControls(page)
...
test('unscheduled safety rest remains passive guidance without claiming scheduled rest', async ({ page }) => {
  const runtimeErrors = collectRuntimeErrors(page)
  const apiState = await installAuthenticatedApi(page, {
    responses: new Map([
  ...
  await expect(page.getByRole('button', { name: /^(?:Start run|Start lift|Start workout|Map route)$/i })).toHaveCount(0)
  await expectNoRoutineCheckInControls(page)
  expect(requestsFor(apiState, 'POST', '/api/checkin')).toHaveLength(0)
...
test('passive safety recovery remains guidance and never offers subjective controls or the rest-labelled run', async ({ page }) => {
  const runtimeErrors = collectRuntimeErrors(page)
  const apiState = await installAuthenticatedApi(page, {
    responses: new Map([
...
test('legacy empty safety rest stays truthful and closes every workout handoff without subjective controls', async ({ page }) => {
  const runtimeErrors = collectRuntimeErrors(page)
  const apiState = await installAuthenticatedApi(page, {
    responses: new Map([
...
test('legacy flat all-removed day stays removed without subjective controls or recovery attribution', async ({ page }) => {
  const runtimeErrors = collectRuntimeErrors(page)
  const apiState = await installAuthenticatedApi(page, {
    responses: new Map([
...
test('the retired check-in route cannot mutate a passive safety-rest plan', async ({ page }) => {
  const runtimeErrors = collectRuntimeErrors(page)
  const apiState = await installAuthenticatedApi(page, {
    responses: new Map([
      ['GET /api/checkin/today', {
  ...
  expect(requestsFor(apiState, 'POST', '/api/checkin')).toHaveLength(0)
  expect(requestsFor(apiState, 'POST', '/api/checkin/preview')).toHaveLength(0)
  assertCleanApiAndRuntime(apiState, runtimeErrors)
})

test('passive lift-only safety rest cannot expose strength or workout starts even with a stale lift payload', async ({ page }) => {
  const runtimeErrors = collectRuntimeErrors(page)
  const apiState = await installAuthenticatedApi(page, {
    responses: new Map([
      ['GET /api/checkin/today', {
...
test('accepted plan hands off through warm-up, run save, and factual recap reload without questionnaires', async ({ page }) => {
  const runtimeErrors = collectRuntimeErrors(page)
  let savedRun = null
  const apiState = await installAuthenticatedApi(page, {
    user: { sex: 'female' },
  ...
  await expect(page.getByRole('button', { name: 'Start Scheduled Run', exact: true })).toBeVisible()
  await expectNoRoutineCheckInControls(page)
  ...
  await expect(page.getByText('Your warm-up is complete. Start the run when you are ready.', { exact: true })).toBeVisible()
  await expect(page.getByText('Your check-in is saved.', { exact: false })).toHaveCount(0)
