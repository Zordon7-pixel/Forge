...
test('rest-day brief and Weekly Recap share one interaction system at mobile widths', async ({ page }, testInfo) => {
  ...
  const todayHeading = page.getByRole('heading', { name: "Review today's plan" })
  await expect(todayHeading).toBeVisible()
  const todayCard = todayHeading.locator('xpath=ancestor::section[1]')
  await expect(todayCard).toHaveClass(/card-hero/)
