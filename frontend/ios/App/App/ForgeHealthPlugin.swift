...
public class ForgeHealthPlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    @objc func getSummary(_ call: CAPPluginCall) {
        ...
        group.notify(queue: .main) {
            ...
            payload["runningPowerWatts"] = runningDynamics.series["running_power_watts"]?.average.map { Int($0.rounded()) } ?? NSNull()
            payload["runningSpeedMps"] = runningDynamics.series["running_speed_mps"]?.average.map { round($0 * 100) / 100 } ?? NSNull()
            payload["runningStrideLengthM"] = runningDynamics.series["running_stride_length_m"]?.average.map { round($0 * 100) / 100 } ?? NSNull()
            payload["runningVerticalOscillationCm"] = runningDynamics.series["running_vertical_oscillation_cm"]?.average.map { round($0 * 10) / 10 } ?? NSNull()
            payload["runningGroundContactTimeMs"] = runningDynamics.series["running_ground_contact_time_ms"]?.average.map { Int($0.rounded()) } ?? NSNull()
