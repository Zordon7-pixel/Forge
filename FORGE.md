...
| Apple Health / Apple Watch | Native HealthKit bridge in `ForgeHealthPlugin.swift` | `health_sync`, imported `runs`/`lifts` | Settings sync, Body metrics, activity history, recent run load, recovery, cardio fitness, full run recaps, routes, elevation, heart rate, pace, power, cadence, and running form | Schema-v6 full-fidelity workout streams are patched and locally verified; Railway deploy, approved EAS build, and physical-iPhone QA have not started |
...
- Status is `patched`. Railway deploy, production verification, EAS/TestFlight build, and physical-iPhone verification have not started.
...
- Native import version 6 triggers one full workout-history re-read after the next approved iPhone build is installed. The existing recent-history refresh continues to recover late-attached HealthKit details afterward.
- Local verification includes the frontend production build, iOS native contract smoke, Swift parse, focused Apple Watch/import/recap regressions, the backend/frontend smoke suites, both zero-vulnerability audits, and complete 61-table account-data export/deletion coverage. The iOS 26.5 runtime was installed, but Apple's available runtime build (`23F73`) does not match this Xcode installation's SDK build (`23F81a`), so Interface Builder still blocks the local archive; the EAS release archive is the authoritative native compile gate. A real Apple Health sync still requires the approved native build and a physical iPhone.
