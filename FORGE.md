...
- Latest verified application release: commit `c4a7f374`, deployment `60250ec2-c60a-4dfe-b6ba-7c715407b711`
- Latest checked bundle: `/assets/index-CN1YnohK.js`
...
- Community now includes a private monthly running-mileage leaderboard for the signed-in athlete and accepted friends only. Contact suggestions are consent-based and email-only: both accounts opt in, address-book values are checked transiently and never stored, and an encrypted short-lived token creates a pending request without exposing the matched email or account id. Native contact reading was included in rejected build 17 and will reach testers in the next approved replacement build; phone verification remains pending.
- Exact-handle friend search is case-insensitive at both input normalization and the SQL boundary. The Friends screen explains that the target athlete must save their handle and opt into handle discovery; hidden accounts continue to receive the same privacy-safe unavailable response.
- Branded post-run share cards, standard race recognition from one mile through marathon, in-app activity alerts, and the Strava webhook ingestion path shipped on 2026-07-20. Strava currently reports the API application as `Inactive` and rejects webhook registration with `403`, so automatic Strava delivery remains provider-blocked; manual sync is unchanged. Native HealthKit background alerts are staged but require a separately approved EAS build.
...
| Apple Health / Apple Watch | Native HealthKit bridge in `ForgeHealthPlugin.swift` | `health_sync`, imported `runs`/`lifts` | Settings sync, Body metrics, activity history, recent run load, recovery, cardio fitness, full run recaps, routes, elevation, heart rate, pace, power, cadence, and running form | Schema v6 is Railway-live and shipped in TestFlight build 20; Apple processing and physical-iPhone verification remain pending |
...
## Native TestFlight Build 20 / Apple Watch Full-Fidelity Recaps (2026-08-15)

- Forged Hybrid `1.0.5 (20)` was built from `e4ab2ea4` with the existing EAS certificate, provisioning profiles, and App Store Connect API key. EAS build `ecd258a4-0b01-44c3-bff7-60a330242f9c` finished successfully. An earlier archive attempt `47d1e69e-07c7-4591-a0cf-102d9cd44b66` failed at native compile, produced no TestFlight artifact, and led to the corrected metric-series accessor in `e4ab2ea4`.
- The signed IPA was inspected after build: version `1.0.5`, build `20`, strict code-signature verification, and the compiled schema-v6 running-power stream marker matched the reviewed source.
- EAS submission `9d926a9f-e8f5-4934-b61f-0a03080863d1` uploaded the binary successfully to App Store Connect. Apple is processing it for TestFlight; physical-iPhone Apple Health permission, reread, route, and recap verification remain pending.
- Railway served feature revision `6d10a9bd` during the production matrix. A disposable account imported one Apple workout without a route, enriched that same canonical run later with three GPS points, returned the full route and heart-rate/power streams in owner detail, omitted large streams from list responses, and was deleted successfully.
- Frontend production build, 64 frontend smokes, 75 backend smokes, iOS native contract, Swift parse, local device-SDK typecheck/link/app validation, both zero-vulnerability audits, privacy metadata, 61-table account-data coverage, production late-route enrichment, EAS archive, and signed-IPA checks passed. Apple's public iOS runtime build still differs from this Mac's SDK patch, so the complete local UI-resource archive remains unavailable; the EAS release archive succeeded.

## Apple Watch Full-Fidelity Run Recaps / Native Schema v6 (2026-08-15)

- Status is `shipped / awaiting device verification`. Railway production and the late-route enrichment matrix passed; TestFlight build 20 is uploaded and processing. Physical-iPhone verification has not yet occurred.
- The HealthKit bridge now imports bounded per-workout timelines for heart rate, post-workout heart-rate recovery, running speed/pace, power, cadence, stride length, vertical oscillation, and ground-contact time, alongside the existing GPS route and elevation data.
- The authenticated health-import boundary validates and idempotently merges those streams with scalar average/minimum/maximum values. Existing details survive a partial later sync, while newly available Apple Health records can enrich the canonical run without creating a duplicate.
- Owner-scoped run detail returns the full data for recap charts. History list responses omit the large stream payload, preserving mobile response size; the recap adds route elevation, heart-rate/recovery, pace, power, cadence, vertical-oscillation, ground-contact, and stride-length traces.
- The missing-data notice no longer claims that the watch omitted details when Forge may simply lack Health permission or may have synced before Apple attached late workout records.
- Native import version 6 triggers one full workout-history re-read after build 20 is installed. The existing recent-history refresh continues to recover late-attached HealthKit details afterward.
- Local verification includes the frontend production build, iOS native contract smoke, Swift parse, device-SDK typecheck/link/app validation, focused Apple Watch/import/recap regressions, the backend/frontend smoke suites, both zero-vulnerability audits, and complete 61-table account-data export/deletion coverage. The iOS 26.5 runtime was installed, but Apple's available runtime build (`23F73`) does not match this Xcode installation's SDK build (`23F81a`), so Interface Builder still blocks a complete local UI-resource archive. The EAS release archive compiled successfully. A real Apple Health sync still requires physical-iPhone verification.
...
| Prescribed lift exercises lacked an accessible form guide or showed a mechanically different substitute image | frontend/src/components/ExerciseGuideAction.jsx, frontend/src/components/MovementDemo.jsx, backend/src/lib/exerciseImageRequests.js | Workout surfaces expose a focus-trapped `View how` guide with exact prescription details, profile-matched vetted visuals, and a truthful written-cue fallback that queues missing exact images for review |
...
| 2026-07-17 | `60250ec2-c60a-4dfe-b6ba-7c715407b711` | Success | Case-insensitive exact-handle lookup and discovery guidance verified with lowercase, uppercase, mixed-case, hidden-target, and complete disposable-account cleanup checks on `/assets/index-CN1YnohK.js`; no EAS build was run |
