...
const WearConnectivityModule = isTurboModuleEnabled ? require('./NativeWearConnectivity').default : NativeModules.WearConnectivity;
const WearConnectivity = WearConnectivityModule ? WearConnectivityModule : new Proxy({}, {
  get() {
 ...
 */
export function monitorFileTransfers(callback) {
  const subscription = DeviceEventEmitter.addListener('FileTransferEvent', callback);
  return () => {
    subscription.remove();
  };
}
...
AppRegistry.registerHeadlessTask('WearConnectivityTask', () => WearConnectivityTask);
