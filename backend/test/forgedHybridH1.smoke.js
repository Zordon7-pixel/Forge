...
assert(plansRouteSource.includes('withPlanningInputMutation(req.user.id, async (tx)') && /getActivePlanForMutation\(req\.user\.id, tx, \{ planningDateLocal: planningDateISO \}\)/.test(plansRouteSource) && plansRouteSource.includes('FOR UPDATE OF up') && plansRouteSource.includes('FOR UPDATE OF tp'), 'make-up reschedule versions the input and locks the owner, assignment, and phone-local current plan before mutation');
