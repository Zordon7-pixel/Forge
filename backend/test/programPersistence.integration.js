...
async function main() {
  ...
  for (const route of ['auth','races','runs','plans']) app.use(`/api/${route}`, require(`../src/routes/${route}`));
  ...
  assert.ok([4, 7].includes(frequency), 'Only the explicit complete-program acceptance frequencies are supported');
  ...
  const profile = await request('PUT', '/auth/me/profile', { run_days_per_week: frequency, lift_days_per_week: frequency,
  ...
  const generationRequest = {
    ...
    target: { runDaysPerWeek: frequency, liftDaysPerWeek: frequency, liftingEnabled: true, planMode: 'hybrid_maintain',
  ...
  assert.equal(fullWeek.days.flatMap(day => day.sessions).filter(session => session.kind === 'lift').length, frequency);
  ...
  if (process.env.PROGRAM_TEST_REMOVAL === '1') {
    ...
    assert.ok(removed.elapsed_ms < 90000, 'Removal preview fits its scoped client deadline');
