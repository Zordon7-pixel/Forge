...
router.get('/compliance', auth, (req, res) => {
  ...
  const runs = db.prepare('SELECT id, date, distance_miles FROM runs WHERE user_id=? AND date>=? AND date<?').all(req.user.id, weekStart, weekEnd);
  const lifts = db.prepare('SELECT id, date FROM lifts WHERE user_id=? AND date>=? AND date<?').all(req.user.id, weekStart, weekEnd);
...
router.post('/reschedule-missed', auth, (req, res) => {
  ...
  const fromDate = originalDate || dayToDate(plan.week_start, source.day);
  ...
  db.prepare('UPDATE training_plans SET plan_json=? WHERE id=?').run(JSON.stringify(parsed), plan.id);
...
router.post('/race-adjust', auth, async (req, res) => {
  ...
  const race = db.prepare('SELECT * FROM race_events WHERE id=? AND user_id=?').get(raceId, req.user.id);
  ...
  db.prepare('UPDATE training_plans SET plan_json=? WHERE id=?').run(JSON.stringify(nextPlan), plan.id);
