...
module.exports = function autoUpdatePRs(db, userId, run) {
  ...
  const upsert = (candidate) => {
    ...
    const existing = db.prepare(`SELECT * FROM personal_records WHERE user_id = ? AND category = 'run' AND label = ?`).get(userId, candidate.label)
    if (!existing) {
      const id = uuidv4()
      db.prepare(`INSERT INTO personal_records (id, user_id, category, label, value, unit, run_id, achieved_at, source, discrepancy, auto_value) VALUES (?, ?, 'run', ?, ?, ?, ?, ?, 'auto', 0, NULL)`).run(
        id,
        userId,
        candidate.label,
        candidate.value,
    ...
    if (existing.source === 'auto') {
      if (isBetter(candidate.value, Number(existing.value), candidate.direction)) {
        db.prepare(`UPDATE personal_records SET value = ?, unit = ?, run_id = ?, achieved_at = ?, discrepancy = 0, auto_value = NULL, source = 'auto' WHERE id = ?`).run(
          candidate.value,
          candidate.unit,
          run.id,
          runDate,
    ...
    if (existing.source === 'manual' && isBetter(candidate.value, Number(existing.value), candidate.direction)) {
      db.prepare(`UPDATE personal_records SET discrepancy = 1, auto_value = ? WHERE id = ?`).run(candidate.value, existing.id)
