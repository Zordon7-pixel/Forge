...
const EXERCISES = [
  ...
  { name: 'Hammer Curl', muscle_group: 'arms', secondary_muscles: 'biceps, brachialis', instructions: 'Hold dumbbells at your sides with palms facing each other (neutral grip). Curl up keeping the neutral grip throughout. Good for building thickness in the bicep and forearm.', how_to_image_url: '' },
  ...
  { name: 'Dips (Tricep)', muscle_group: 'arms', secondary_muscles: 'triceps, chest', instructions: 'Use parallel bars or a bench. Keep your body upright (not leaning forward). Lower yourself by bending your elbows to 90 degrees, then press back up. Upright position targets triceps more than chest.', how_to_image_url: '' },
...
function seedExercises() {
  const existing = db.prepare('SELECT COUNT(*) as c FROM exercises').get().c;
  if (existing > 0) {
    console.log(`Exercises already seeded (${existing} records) — skipping`);
    return;
  }
  const insert = db.prepare(`
    INSERT OR IGNORE INTO exercises (name, muscle_group, secondary_muscles, instructions, how_to_image_url, is_system)
    VALUES (@name, @muscle_group, @secondary_muscles, @instructions, @how_to_image_url, 1)
  `);
...
if (require.main === module) seedExercises();
