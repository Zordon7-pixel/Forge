...
ALTER TABLE plan_generation_candidates ADD COLUMN IF NOT EXISTS decision_id TEXT;
ALTER TABLE plan_generation_candidates ADD COLUMN IF NOT EXISTS candidate_revision BIGINT CHECK (candidate_revision IS NULL OR candidate_revision >= 1);
ALTER TABLE plan_generation_candidates ADD COLUMN IF NOT EXISTS athlete_state_revision BIGINT CHECK (athlete_state_revision IS NULL OR athlete_state_revision >= 1);
ALTER TABLE plan_generation_candidates ADD COLUMN IF NOT EXISTS safety_state_hash TEXT;
ALTER TABLE plan_generation_candidates ADD COLUMN IF NOT EXISTS goal_revisions_json JSONB CHECK (goal_revisions_json IS NULL OR pg_column_size(goal_revisions_json) <= 16384);
ALTER TABLE plan_generation_candidates ADD COLUMN IF NOT EXISTS lock_revision BIGINT CHECK (lock_revision IS NULL OR lock_revision >= 0);
ALTER TABLE plan_generation_candidates ADD COLUMN IF NOT EXISTS edit_revision BIGINT CHECK (edit_revision IS NULL OR edit_revision >= 0);
ALTER TABLE plan_generation_candidates ADD COLUMN IF NOT EXISTS surface_revision BIGINT CHECK (surface_revision IS NULL OR surface_revision >= 1);
ALTER TABLE plan_generation_candidates ADD COLUMN IF NOT EXISTS export_revision BIGINT CHECK (export_revision IS NULL OR export_revision >= 1);
ALTER TABLE plan_generation_candidates ADD COLUMN IF NOT EXISTS feature_mode TEXT CHECK (feature_mode IS NULL OR feature_mode IN ('off', 'shadow', 'preview', 'on'));
ALTER TABLE plan_generation_candidates ADD COLUMN IF NOT EXISTS selected_candidate_hash TEXT;
ALTER TABLE plan_generation_candidates ADD COLUMN IF NOT EXISTS material_change_json JSONB CHECK (material_change_json IS NULL OR pg_column_size(material_change_json) <= 16384);
...
CREATE INDEX IF NOT EXISTS idx_pipeline_artifacts_user_decision_kind
  ON planning_pipeline_artifacts(user_id, decision_id, artifact_kind);
CREATE INDEX IF NOT EXISTS idx_pipeline_artifacts_candidate_kind
  ON planning_pipeline_artifacts(plan_generation_candidate_id, artifact_kind);
CREATE INDEX IF NOT EXISTS idx_pipeline_artifacts_user_kind_created
  ON planning_pipeline_artifacts(user_id, artifact_kind, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_evidence_corrections_user_evidence_revision
  ON planning_evidence_corrections(user_id, raw_evidence_kind, raw_evidence_ref, revision DESC);
...
CREATE INDEX IF NOT EXISTS idx_planning_constraints_user_active_kind
  ON planning_constraints(user_id, active, constraint_kind);
CREATE INDEX IF NOT EXISTS idx_planning_constraints_user_plan_revision
  ON planning_constraints(user_id, plan_id, revision DESC);
CREATE UNIQUE INDEX IF NOT EXISTS idx_planning_constraints_scope_revision
  ON planning_constraints(user_id, constraint_kind, COALESCE(plan_id, ''), COALESCE(session_id, ''), COALESCE(date_local, ''), revision);
...
CREATE INDEX IF NOT EXISTS idx_candidate_rejections_user_created
  ON plan_candidate_rejections(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_candidate_rejections_user_decision
  ON plan_candidate_rejections(user_id, decision_id);
...
CREATE INDEX IF NOT EXISTS idx_community_posts_user_run ON community_posts(user_id, run_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_plan_adjustment_proposals_episode ON plan_adjustment_proposals(user_id, episode_key) WHERE episode_key IS NOT NULL AND trigger_run_id IS NULL;
...
-- Additive manual measurement successors. No backfill or plan mutation.
CREATE TABLE IF NOT EXISTS activity_measured_receipts (
  id TEXT PRIMARY KEY,
  user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
