...
async function initDb() {
  ...
  try {
    ...
    const userColumnMigrations = [
      ...
      'ALTER TABLE users ADD COLUMN IF NOT EXISTS contact_discoverable INTEGER DEFAULT 0',
    ...
    `);
    await client.query('CREATE INDEX IF NOT EXISTS idx_activity_import_claims_activity ON activity_import_claims(user_id, activity_kind, activity_id)');
    ...
    `);
    await client.query('CREATE INDEX IF NOT EXISTS idx_shoe_catalog_brand_model ON shoe_catalog(LOWER(brand), LOWER(model))');
    ...
    await client.query("ALTER TABLE gear_shoes ADD COLUMN IF NOT EXISTS surface TEXT DEFAULT 'road'");
    await client.query("ALTER TABLE gear_shoes ADD COLUMN IF NOT EXISTS intent_tags TEXT DEFAULT '[]'");
    await client.query('ALTER TABLE gear_shoes ADD COLUMN IF NOT EXISTS wet_ok INTEGER');
    await client.query('ALTER TABLE gear_shoes ADD COLUMN IF NOT EXISTS cushion TEXT');
    await client.query('ALTER TABLE gear_shoes ADD COLUMN IF NOT EXISTS catalog_id TEXT');
    await client.query(`
      DO $$
      BEGIN
        IF NOT EXISTS (
      ...
      END $$
    `);
    await client.query('CREATE INDEX IF NOT EXISTS idx_gear_shoes_catalog_id ON gear_shoes(catalog_id)');
    ...
    `);
    await client.query('CREATE INDEX IF NOT EXISTS idx_group_runs_owner_status_start ON group_runs(owner_id, status, starts_at)');
    await client.query('CREATE INDEX IF NOT EXISTS idx_group_runs_status_start ON group_runs(status, starts_at)');
    ...
    `);
    await client.query('CREATE INDEX IF NOT EXISTS idx_group_run_members_run_status ON group_run_members(group_run_id, status)');
    await client.query('CREATE INDEX IF NOT EXISTS idx_group_run_members_user_status ON group_run_members(user_id, status)');
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS race_events (
        ...
        event_kind TEXT NOT NULL DEFAULT 'run_race',
        event_format TEXT,
        event_category TEXT,
        event_local_date TEXT,
        event_timezone TEXT,
        rules_version TEXT,
        event_config_json JSONB,
        location TEXT,
